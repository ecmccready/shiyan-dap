...
export default function MarketplacePage() {
  ...
  useEffect(() => {
    fetch("/api/playlist")
      .then((res) => res.json())
      .then((data) => setBundles(data.playlists || []))
      .catch(() => {});
  }, []);
      ...
      <header className="border-b border-zinc-800/80">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-6">
            ...
            <Link href="/measurements" className="text-sm text-zinc-400 hover:text-white">Measurements</Link>
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        <div className="mb-10">
          ...
          <div className="flex flex-wrap gap-3 mt-5">
            ...
            <Link href="/home" className="h-9 px-4 rounded-full bg-zinc-800 text-white text-xs font-medium flex items-center">
              Contained Homes
            </Link>
          </div>
        </div>
        ...
        {bundles.length > 0 && (
          <div className="mb-10 bg-zinc-900/60 border border-amber-800/40 rounded-2xl p-6">
            <p className="text-sm text-amber-400 mb-2">B2B bundle</p>
            {bundles.map((bundle) => (
              <div key={bundle.id} className="mb-2">
                <p className="text-lg font-semibold">{bundle.name}</p>
                <p className="text-sm text-zinc-400">
                  {bundle.productType} · {bundle.license} ·{" "}
                  {bundle.clusterIds?.length || 0} clusters ·{" "}
                  {bundle.b2bReady ? "ready for license" : "assembling"}
                </p>
                <Link
                  href="/playlist"
                  className="inline-block mt-3 h-9 px-4 rounded-full bg-amber-600 text-white text-xs font-medium leading-9"
                >
                  License this playlist
                </Link>
              </div>
            ))}
          </div>
              ...
              <div key={cluster.id} className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    {cluster.emergence && (
                      <p className="text-xs text-zinc-500 mt-1">
                        x {cluster.emergence.x} · y {cluster.emergence.y} · z {cluster.emergence.z}
                      </p>
                    )}
                  </div>
                  <span
