...
function ProofInner() {
  ...
  const asset = q.get("asset") || "cl_shiyan_yishu_001";
  ...
  return (
    <main className="min-h-screen bg-black text-zinc-100 p-8 max-w-2xl mx-auto space-y-6">
      <p className="text-xs uppercase tracking-widest text-emerald-400">
        Proof chain · payment ≠ Shiyan state
      </p>
...
z                not claimed from this URL`}
      ...
      <ul className="text-sm text-zinc-400 space-y-1">
        <li>session_id present ≠ paid in Shiyan</li>
...
export default function ProofPage() {
  return (
    <Suspense fallback={<main className="bg-black text-zinc-100 p-8">Proof</main>}>
      <ProofInner />
    </Suspense>
  );
}